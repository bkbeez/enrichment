...
<?php
    ...
    if( isset($_GET['error']) ){
        ...
        if( isset($basic->organization_code) ){
            if( $basic->organization_code=='02' ){
                $log_action = 'login';
                ...
                $_SESSION['loginmsg']['title'] = 'Login Fail !!!';
                $_SESSION['loginmsg']['text'] = 'บัญชีนี้ไม่ใช่นักศึกษาของคณะศึกษาศาสตร์ มช., <i>โปรดตรวจสอบบัญชีของท่าน</i>';
                header('location: '.APP_HOME.'/login/deny.php');
                exit();
            }
        }
